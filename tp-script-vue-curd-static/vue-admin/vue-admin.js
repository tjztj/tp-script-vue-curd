...
define(requires, function ( axios,Qs) {
    ...
    /****窗口方法***/
    window.parseTime=function(t,e){if(0===arguments.length)return null;const n=e||"{y}-{m}-{d} {h}:{i}:{s}";let g;g="object"==typeof t?t:("number"!=typeof(t="string"==typeof t&&/^-?[0-9]+$/.test(t)?parseInt(t):t)||10!==t.toString().length&&9!==t.toString().length||(t*=1e3),new Date(t));const r={y:g.getFullYear(),m:g.getMonth()+1,d:g.getDate(),h:g.getHours(),i:g.getMinutes(),s:g.getSeconds(),a:g.getDay()};return n.replace(/{(y|m|d|h|i|s|a)+}/g,(t,e)=>{let n=r[e];return"a"===e?["日","一","二","三","四","五","六"][n]:(0<t.length&&n<10&&(n="0"+n),n||0)})};
