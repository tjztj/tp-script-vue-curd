...
class CascaderFilter extends ModelFilter
{
    ...
    public function generateWhere(Query $query,$value):void{
        static $listArr = [];
        isset($listArr[$this->guid]) || $listArr[$this->guid] = TreeSelect::treeToList($this->getItems(), $this->valueName, $this->childrenName);
        $list = $listArr[$this->guid];
        ...
        if ($value || 0 === $value || '0' === $value) {
            ...
            if (method_exists($this->field, 'multiple') && $this->field->multiple()) {
                if ($canCheckParent) {
                    $query->whereFindInSet($this->whereName(), $value);
                } else {
                    $query->where(function (Query $q) use ($value, $list) {
                        foreach ($list[$value]['childLastVals'] ?: [$value] as $v) {
                            $q->whereFindInSet($this->whereName(), $v, 'OR');
                        }
                    });
                }
