...
class WeekField extends ModelField
{
    ...
    public function setSave(array $data): self
    {
        ...
        if (isset($data[$name])) {
            ...
            if(empty($data[$name])){
                ...
                $time=strtotime($data[$name]);
                if($time===false){
                    throw new \think\Exception('周时间格式不正确');
                }
                //获取周一日期
                $w=date('w',$time);
                $this->save=strtotime(date('Y-m-d',$time - (($w?:7) - 1) * 24 * 3600));
            }
        }
        return $this;
    }
    ...
    public static function getWeekStr(int $time,bool $showDate=true):string{
        $day=date('d',$time); //今天几号
        //本月1号是星期几
        $begin_w=(int)date('w',strtotime(date('Y-m-01',$time)));
        $begin_w||$begin_w=7;
        if($begin_w!==1){
            //如果不是星期一
            //获取上一个月剩余在这里的天数
            ...
            if($last_day_num>=$day){
                //那么这是上一月的
                return self::getWeekStr(strtotime(date('Y-m-01',$time))-1);
            }else{
                $num=ceil(bcdiv(bcsub($day,$last_day_num),7,1)); //向上取整
                $month=date('Y年m月',$time);
            }
        }else{
        ...
        $end=strtotime(date('Y-m-d',$start).' +6 day');//+7是下周星期一，我要的是本周星期日，所以加6
