...
class FieldCollection extends Collection
{
            ...
            foreach ($v instanceof TreeSelect ? $v->getSourceItems() : $v->items() as $key => $val) {
                ...
                if (!$val['showItemBy']->check($data, true, $old)) {
                    if (isset($data[$v->name()]) && '' !== $data[$v->name()]) {
                        ...
                        } elseif ((string) $data[$v->name()] === (string) $val['value']) {
                            $data[$v->name()] = '';
                        }
                    }
                    unset($items[$key]);
     ...
     * 生成查询条件.
     *
     * @param string|array|null $myFilter 传入参数和自己获取两种方法(json或数组或空)
     *
    ...
    private function getFiledHideListByData(array $data, BaseModel $old, bool $isSourceData = true): array
    {
        ...
        $checkHideField = function (ModelField $field, $checkVal) use ($arrHave, &$data, &$changeFieldHideList, $isSourceData, &$checkHideField, &$fieldHideList, $old) {
            ...
            $vName = $field->name();
            $vType = $field->getType();
            $reversalHideFields = method_exists($field, 'reversalHideFields') && true === $field->reversalHideFields();
            ...
            if (method_exists($field, 'hideFields')) {
                ...
                } elseif (method_exists($field, 'defHideAboutFields') && $field->defHideAboutFields()) { // 默认隐藏所有
            ...
            } elseif (method_exists($field, 'items')) {
                $vValue = $checkVal;
                if (!$isSourceData && !is_null($checkVal)) {// 因为我可能强制设了 $vValue 为null,不让它显示
                    $fieldCopy = clone $field;
                    try {
                ...
                if ($vValue) {
                    ...
                    foreach ($field->items() as $item) {
                        ...
                        $item['hideFields']->each(function (ModelField $hidelField) use ($vType, $vValue, $arrHave, $item, $field, &$hideAllFieldArr, &$hideFieldArr) {
