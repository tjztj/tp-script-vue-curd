...
trait BaseIndex
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            if (1 === $this->request->param('get_left_cate/d', 0)) {
                $leftCate = $this->getLeftCate();
                null === $leftCate || $this->success($leftCate->toArray());
            }
            ...
            try {
                if (is_array($parentInfo)) {
                    ...
                    foreach ($listArr as $k => $vs) {
                        ...
                        $this->fields->each(function (ModelField $v) use ($cList, $bs) {
                            $v->onIndexList($cList, $bs);
                        });
                        FieldDo::doIndex($this->fields, $cList, $bs);
                    }
                } else {
                    $this->fields->each(function (ModelField $v) use ($list, $parentInfo) {
                        $v->onIndexList($list, $parentInfo);
                    });
        ...
        try {
            $this->fields->each(function (ModelField $v) use ($parentInfo) {
                $v->onIndexShow($parentInfo);
            });
        ...
        $showFilter = 1 === $this->request->param('show_filter/d', 1);
        ...
        $showTableTool = 1 === $this->request->param('show_table_tool/d', 1);
        ...
        $leftCate = $this->getLeftCate();
        if ($childTpl && $leftCate && $leftCate->show && 'base_id' === $leftCate->paramName) {
            $leftCate->show = false;
        }
    ...
    protected function indexBeforeSetBase(?BaseModel &$parentInfo): void
    {
        ...
        if (null === $this->getParentController()) {
            // 如果没有父表
            return;
        }
        // 可以在外面赋值
    ...
    protected function indexListModelWhere(BaseModel $model, ?BaseModel $parentInfo, string $searchIdKey = 'id')
    {
        ...
        return $model
            ->where(function (Query $query) {
                $leftCate = $this->getLeftCate();
                if (!$leftCate || !$leftCate->show || 'base_id' === $leftCate->paramName) {
                    return;
                }
            ...
            ->where(function (Query $query) use ($parentInfo, $searchIdKey) {
                ...
                null === $parentInfo || $query->where($this->md::parentField(), $parentInfo->id);
            })
            ->where(function (Query $query) {
                // 这里不应该抛出异常
     ...
     * 列表所有数据的父表设置.
     *
     * @param array|BaseModel|null               $oldBaseInfo
     ...
     * @param array|BaseModel|null               $parentInfo
     *
    ...
    protected function setListDataStep($list, $parentInfo)
    {
        ...
        return $list->map(static function (BaseModel $info) use ($parentInfo, $list, $fields) {
            ...
            null === $stepInfo || $stepInfo = clone $stepInfo;
            $info->stepInfo = $stepInfo ? $stepInfo->listRowDo($info, $parentInfo, $fields)->toArray() : null;

            $nextStepInfo = $fields->getNextStepInfo($info, $parentInfo);
            ...
            if ($stepInfo && (null === $nextStepInfo || $stepInfo->getStep() !== $nextStepInfo->getStep())) {
                ...
                $info->stepCanEdit = $stepInfo->authCheck($info, $parentInfo, $fields, $list);
            } else {
                ...
                if (!isset($btn->url) || is_null($btn->url)) {
                    $btn->url = url(
                        str_replace(
                            ['\\', '._'],
                            ['.', '.'],
                            parse_name(ltrim(str_replace($this->app->getNamespace().'\\controller\\', '', get_class($childControlle)), '\\'))
                        ).'/index',
